const cramer2020TextEu = new Map(Object.entries({
  "← Volver a ejercicios": "← Itzuli ariketetara",
  "2020 · Ordinaria · Opción A · Ejercicio A.1": "2020 · Ohikoa · A aukera · A.1 ariketa",
  "Álgebra · Sistemas lineales · 2020": "Aljebra · Sistema linealak · 2020",
  "Discusión y resolución de un sistema mediante Cramer": "Sistema baten eztabaida eta ebazpena Cramerren bidez",
  ...
  "la edición consultada publica D": "kontsultatutako edizioak D",
  "= 22 − 2a y D": "= 22 − 2a eta D",
  "= 10 − 2a. Al desarrollar ambos determinantes se obtienen D": "= 10 − 2a argitaratzen ditu. Bi determinanteak garatzean D",
  "= 23 − 3a y D": "= 23 − 3a eta D",
  "= 10 − 8a, que son los valores usados aquí.": "= 10 − 8a lortzen dira; hemen balio horiek erabili ditugu.",
...
const originalCramer2020Text = new WeakMap();

const cramer2020Attributes = [
...
function applyCramer2020Text(language) {
  ...
  while (walker.nextNode()) {
    ...
    if (!originalCramer2020Text.has(node)) {
      originalCramer2020Text.set(node, node.nodeValue);
    }

    const original = originalCramer2020Text.get(node);
    const key = original.trim();

    if (language === "eu" && cramer2020TextEu.has(key)) {
      const translation = cramer2020TextEu.get(key);
...
function applyCramer2020Language(language) {
  ...
  applyCramer2020Text(language);

  cramer2020Attributes.forEach(({ selector, attribute, eu, es }) => {
...
document.addEventListener("languagechange", (event) => {
  applyCramer2020Language(event.detail.language);
});

applyCramer2020Language(window.I18n?.getLanguage() ?? "eu");
