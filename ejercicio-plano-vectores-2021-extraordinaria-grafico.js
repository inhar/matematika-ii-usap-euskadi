(() => {
  const BOARD_ID = "plane-extra-2021-board";
  const COLORS = {
    ink: "#1b1b18",
    muted: "#6f746e",
    plane: "#f1f2ef",
  ...
  function initSpanPlaneGraph() {
    ...
    const vectorV = [1, 3, 5];
    const normal = [1, -2, 1];
    const vectorUUnit = normalize(vectorU);
    const vectorVUnit = normalize(vectorV);
    const normalUnit = normalize(normal);
    const projectionOnU = vectorUUnit.map((coordinate) => dot(vectorV, vectorUUnit) * coordinate);
    const secondPlaneDirection = normalize(subtract(vectorV, projectionOnU));

    view.create("polygon3d", [
      planeCorners(pointA, vectorUUnit, secondPlaneDirection, 1.48, 1.02),
    ], {
    ...
    view.create("line3d", [pointA, vectorUUnit, [0.14, 1.08]], vectorStyle(COLORS.accent, 3));
    view.create("line3d", [pointA, vectorVUnit, [0.14, 1.32]], vectorStyle(COLORS.secondary, 2.8));
    view.create("line3d", [pointA, normalUnit, [0.15, 1.12]], {
    ...
    view.create("text3d", [add(add(pointA, vectorUUnit, 0.68), secondPlaneDirection, 0.13), "u"], {
      ...textStyle,
      color: COLORS.accent,
    });
    const vectorVLabelPosition = add(
      add(add(pointA, vectorVUnit, 0.85), secondPlaneDirection, 0.02),
      normalUnit,
      -0.15,
    );
    view.create("text3d", [vectorVLabelPosition, "v"], {
      ...textStyle,
      color: COLORS.secondary,
      anchorY: "top",
    });
    view.create("text3d", [add(add(pointA, normalUnit, 0.7), vectorUUnit, 0.62), "n"], {
      ...textStyle,
      color: COLORS.ink,
    });
    view.create("text3d", [add(add(pointA, vectorUUnit, -1.05), secondPlaneDirection, 0.72), "π"], {
      ...textStyle,
      color: COLORS.accent,
      fontSize: 25,
    });
    ...
    window.planeExtra2021Board = board;
  }
